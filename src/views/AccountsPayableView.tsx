...
export default function AccountsPayableView() {
  ...
  const [recurrence, setRecurrence] = useState<'Nenhuma' | 'Semanal' | 'Mensal' | 'Trimestral' | 'Anual'>('Nenhuma');
  ...
  const handleCancel = (id: string) => {
    if (window.confirm('Deseja realmente cancelar este lançamento? O registro histórico será preservado para auditoria.')) {
  ...
  return (
    <div id="accounts-payable-root" className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 id="payable-title" className="text-xl font-bold text-zinc-900 tracking-tight">Contas a Pagar</h2>
          <p className="text-zinc-500 text-xs">Gestão de compromissos, agendamentos, validação de boletos e histórico de liquidações.</p>
        </div>
        ...
        {hasPermission('accounts-payable.create') && (
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-bold text-white bg-[#d20010] hover:bg-[#850000] px-4 py-2.5 rounded-lg transition-colors cursor-pointer shadow-xs"
          >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl border border-zinc-200 shadow-2xl max-w-xl w-full overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="p-5 border-b border-zinc-100 bg-gradient-to-r from-[#00304c] to-[#d20010] text-white flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold">Lançar Nova Conta a Pagar</h3>
                <p className="text-[10px] text-[#ffefd1]">Dividido em 3 etapas de verificação operacional para BPO.</p>
              </div>
              <button onClick={resetForm} className="text-[#ffefd1] hover:text-white font-bold text-xs cursor-pointer">Fechar</button>
            </div>

            {/* Steps Indicator bar */}
            <div className="flex bg-[#00304c] border-b border-[#00304c]/20 px-5 py-3.5 text-xs justify-between font-medium">
              <span className={`flex items-center gap-1.5 ${formStep >= 1 ? 'text-white font-extrabold' : 'text-white/40'}`}>
                <span className="h-5 w-5 rounded-full bg-[#d20010] text-white flex items-center justify-center text-[10px] font-black">1</span> Fornecedor
              </span>
              <span className={`flex items-center gap-1.5 ${formStep >= 2 ? 'text-white font-extrabold' : 'text-white/40'}`}>
                <span className={`h-5 w-5 rounded-full flex items-center justify-center text-[10px] font-black ${formStep >= 2 ? 'bg-[#d20010] text-white' : 'bg-[#002236] text-white/40'}`}>2</span> Valores
              </span>
              <span className={`flex items-center gap-1.5 ${formStep >= 3 ? 'text-white font-extrabold' : 'text-white/40'}`}>
                <span className={`h-5 w-5 rounded-full flex items-center justify-center text-[10px] font-black ${formStep >= 3 ? 'bg-[#d20010] text-white' : 'bg-[#002236] text-white/40'}`}>3</span> Liquidação
              </span>
            </div>

            <form onSubmit={formStep === 3 ? handleSubmit : handleNextStep} className="p-6 space-y-4">
              {/* STEP 1: Fornecedor e Classificação */}
              {formStep === 1 && (
                <div className="space-y-4 animate-in slide-in-from-right-5 duration-150">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase block">Descrição da Conta *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase block">Fornecedor / Beneficiário *</label>
                    <input
                      type="text"
                      required
                      placeholder="Ex: Amazon Web Services Ltda"
                      className="w-full p-2 text-xs bg-zinc-50 border border-zinc-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-zinc-900"
                      value={supplier}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase block">Categoria de Plano *</label>
                      <select
                        required
                      ...
                      >
                        ...
                        <option value="Aluguel e IPTU">Aluguel e IPTU</option>
                        <option value="Insumos e Matérias-primas">Insumos e Matérias-primas</option>
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        <option value="">Selecione...</option>
                        <option value="Operações e P&D">Operações e P&D</option>
              ...
              {formStep === 2 && (
                <div className="space-y-4 animate-in slide-in-from-right-5 duration-150">
                  <div className="grid grid-cols-3 gap-3">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase block">Mês Competência</label>
                      <input
                        type="month"
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase block">Data Emissão</label>
                      <input
                        type="date"
                        className="w-full p-2 text-xs bg-zinc-50 border border-zinc-200 rounded-lg"
                        value={issueDate}
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase block">Data Vencimento *</label>
                      <input
                        type="date"
                        required
                        className="w-full p-2 text-xs bg-zinc-50 border border-zinc-200 rounded-lg"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase block">Valor Principal (R$) *</label>
                      <input
                        type="number"
                        step="0.01"
                        required
                  ...
                  <div className="p-3 bg-zinc-50 rounded-lg text-[10px] text-zinc-500 font-medium">
                    Previsão de Valor Líquido Final: <strong>R$ {(Number(amount) + Number(interest) + Number(penalty) - Number(discount)).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</strong>.<br />
                    {Number(amount) >= activeCompany.approvalLimit && (
                      <span className="text-amber-600">Este valor atinge ou excede o limite de aprovação (R$ {activeCompany.approvalLimit.toLocaleString('pt-BR')}) e exigirá autorização do cliente.</span>
                    )}
                  </div>
                </div>
              )}
              ...
              {formStep === 3 && (
                <div className="space-y-4 animate-in slide-in-from-right-5 duration-150">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase block">Método Pagamento</label>
                      <select
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase block">Debitar de Qual Conta *</label>
                      <select
                        required
                        className="w-full p-2 text-xs bg-zinc-50 border border-zinc-200 rounded-lg cursor-pointer"
                        value={bankAccountId}
                      ...
                      >
                        {accounts.map(ba => (
                          <option key={ba.id} value={ba.id}>{ba.bankName} - Saldo R$ {ba.balance.toLocaleString('pt-BR')}</option>
                        ))}
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase block">Recorrência</label>
                      <select
                        className="w-full p-2 text-xs bg-zinc-50 border border-zinc-200 rounded-lg cursor-pointer"
                        value={recurrence}
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase block">Número do Documento / NF</label>
                      <input
                        type="text"
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase block">Anexar Fatura / Boleto (PDF/Imagem)</label>
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => setAttachmentName('boleto_upload_simulado.pdf')}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase block">Observações adicionais</label>
                    <textarea
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-zinc-50 border-b border-zinc-200">
                ...
                <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">Lançamento</th>
                <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">Fornecedor</th>
                <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">Vencimento</th>
                <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider text-right">Valor Líquido</th>
            ...
            <tbody className="divide-y divide-zinc-200 text-xs">
              {filteredPayables.map((ap) => {
                ...
                const isOverdue = new Date(ap.dueDate) < new Date() && ap.status !== 'Paga' && ap.status !== 'Cancelada';
                  ...
                  <React.Fragment key={ap.id}>
                    ...
                    >
                      ...
                      <td className="p-4 font-semibold text-zinc-900">
                        {ap.description}
                        <div className="text-[10px] text-zinc-400 font-normal">Nº: {ap.documentNumber || 'N/A'} | Cat: {ap.category}</div>
                      </td>
                      <td className="p-4 text-zinc-600 font-medium">{ap.supplier}</td>
                      <td className={`p-4 font-medium ${isOverdue ? 'text-rose-600 font-bold' : 'text-zinc-600'}`}>
                        {new Date(ap.dueDate).toLocaleDateString('pt-BR')}
                        {isOverdue && <span className="text-[9px] bg-rose-50 border border-rose-100 text-rose-600 px-1.5 py-0.5 rounded ml-2 font-bold uppercase tracking-wider">Atrasado</span>}
                      </td>
                      <td className="p-4 text-right font-bold text-zinc-900 font-mono">
                        R$ {ap.finalAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </td>
                      <td className="p-4 text-center">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${getStatusBadge(ap.status)}`}>
                          {ap.status}
                        </span>
                      </td>
                      <td className="p-4 text-right" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-2">
                          {ap.status !== 'Paga' && ap.status !== 'Cancelada' && ap.status !== 'Aguardando aprovação' && hasPermission('reconciliation.execute') && (
                            <button
                          ...
                          {ap.status !== 'Paga' && ap.status !== 'Cancelada' && hasPermission('accounts-payable.cancel') && (
                            <button
                    ...
                    {isExpanded && (
                      <tr>
                        <td colSpan={7} className="p-4 bg-zinc-50/50 border-t border-b border-zinc-100">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs text-zinc-600">
                            ...
                            <div className="space-y-2">
                              <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1">
                                <DollarSign className="h-3.5 w-3.5" /> Detalhamento de Valores
                              </h4>
                              <div className="space-y-1 bg-white p-3 rounded-lg border border-zinc-200/60 font-mono">
                                <div className="flex justify-between">
                                  <span>Valor Principal:</span>
                                  <span>R$ {ap.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                </div>
                                <div className="flex justify-between text-rose-500">
                                  <span>Juros:</span>
                                  <span>+ R$ {ap.interest.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                </div>
                                <div className="flex justify-between text-rose-500">
                                  <span>Multa:</span>
                                  <span>+ R$ {ap.penalty.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                </div>
                                <div className="flex justify-between text-emerald-600">
                                  <span>Desconto:</span>
                                  <span>- R$ {ap.discount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                </div>
                                <div className="flex justify-between font-bold text-zinc-900 border-t border-zinc-100 pt-1 text-sm">
                                  <span>Líquido Final:</span>
                                  <span>R$ {ap.finalAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="space-y-2">
                              <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1">
                                <Clock className="h-3.5 w-3.5" /> Metadados de Liquidação
                              </h4>
                              <div className="space-y-1.5 bg-white p-3 rounded-lg border border-zinc-200/60">
                                <div>
                                  <span className="text-zinc-400 font-medium block text-[9px] uppercase">Forma de Pagamento</span>
                                  <span className="font-bold text-zinc-800">{ap.paymentMethod}</span>
                                </div>
                                <div>
                                  <span className="text-zinc-400 font-medium block text-[9px] uppercase">Recorrência</span>
                                  <span className="font-semibold text-zinc-800">{ap.recurrence}</span>
                                </div>
                                <div>
                                  <span className="text-zinc-400 font-medium block text-[9px] uppercase">Conta Bancária Origem</span>
                                  <span className="font-semibold text-zinc-800">
                                    {bankAccounts.find(ba => ba.id === ap.bankAccountId)?.bankName || 'Itaú'}
                                  </span>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="space-y-2">
                              <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1">
                                <Paperclip className="h-3.5 w-3.5" /> Documentos & Auditoria
                              </h4>
                              <div className="space-y-2 bg-white p-3 rounded-lg border border-zinc-200/60">
                                <div>
                                  <span className="text-zinc-400 font-medium block text-[9px] uppercase">Anexo Cadastrado</span>
                                  {ap.attachmentName ? (
                                    <a 
                                      href="#" 
                                      onClick={(e) => { e.preventDefault(); alert('Abrindo documento com token seguro assinado por S3...'); }}
                                    ...
                                    >
                                      ...
                                      {ap.attachmentName} <ExternalLink className="h-3 w-3 inline text-zinc-400" />
                                    </a>
                                  ) : (
                                    <span className="text-zinc-400 italic">Nenhum anexo enviado</span>
                                  )}
                                </div>

                                {ap.status === 'Paga' && (
                                  <div>
                                    <span className="text-zinc-400 font-medium block text-[9px] uppercase">Comprovante de Baixa</span>
                                    <span className="text-emerald-600 font-semibold flex items-center gap-1 mt-0.5">
                                      <CheckCircle className="h-3.5 w-3.5 text-emerald-500" /> Confirmado em {new Date(ap.paymentDate || '').toLocaleDateString('pt-BR')}
                                    </span>
                                  </div>
                                )}
                                ...
                                <div className="border-t border-zinc-100 pt-1.5 text-[10px] text-zinc-400 font-mono leading-tight">
                                  UUID: {ap.id}<br />
                                  Registrado: {new Date(ap.createdAt).toLocaleString('pt-BR')}
                                </div>
                              </div>
                            </div>
                          </div>
              ...
              {filteredPayables.length === 0 && (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-zinc-400 italic">
                    Nenhuma conta a pagar encontrada correspondente aos termos de busca.
                  </td>
                </tr>
              )}
            </tbody>
