...
export default function CashFlowView() {
  ...
  const payables = accountsPayable.filter(ap => ap.companyId === activeCompany.id && ap.status !== 'Cancelada');
  const receivables = accountsReceivable.filter(ar => ar.companyId === activeCompany.id && ar.status !== 'Cancelada');
  ...
  const getTimelineData = () => {
    const data: Record<string, { date: string, incomingRealized: number, incomingProjected: number, outgoingRealized: number, outgoingProjected: number }> = {};
    
    // Add receivables
    receivables.forEach(ar => {
      // filters
      if (selectedAccount !== 'ALL' && ar.bankAccountId !== selectedAccount) return;
      if (selectedCategory !== 'ALL' && ar.category !== selectedCategory) return;
      if (selectedCostCenter !== 'ALL' && ar.costCenter !== selectedCostCenter) return;

      const date = ar.receiptDate || ar.dueDate;
      if (!data[date]) {
        data[date] = { date, incomingRealized: 0, incomingProjected: 0, outgoingRealized: 0, outgoingProjected: 0 };
      }

      if (ar.status === 'Recebida') {
    ...
    payables.forEach(ap => {
      // filters
      if (selectedAccount !== 'ALL' && ap.bankAccountId !== selectedAccount) return;
      if (selectedCategory !== 'ALL' && ap.category !== selectedCategory) return;
      if (selectedCostCenter !== 'ALL' && ap.costCenter !== selectedCostCenter) return;
  ...
  const getGroupedPeriods = () => {
    ...
    if (periodType === 'monthly') {
      const groups: Record<string, { period: string, inRealized: number, inProjected: number, outRealized: number, outProjected: number }> = {};
      
      cashFlowTimeline.forEach(item => {
        const monthYear = item.rawDate.substring(0, 7); // YYYY-MM
        if (!groups[monthYear]) {
          groups[monthYear] = { period: monthYear, inRealized: 0, inProjected: 0, outRealized: 0, outProjected: 0 };
        }
  ...
  const handleExport = () => {
    ...
    generateReport(`Fluxo de Caixa Realizado vs Projetado`, 'Fluxo de Caixa', filterSummary);
  };
  ...
  return (
    <div id="cash-flow-root" className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 id="cashflow-title" className="text-xl font-bold text-zinc-900 tracking-tight">Fluxo de Caixa Operacional</h2>
          <p className="text-zinc-500 text-xs">Visão integrada das movimentações financeiras executadas e previsões futuras de caixa.</p>
        </div>
      ...
      {/* Multi-Filter Bar */}
      <div className="bg-white rounded-xl border border-zinc-200 shadow-xs p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
        {/* Account selection */}
        <div className="space-y-1">
          <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Conta Bancária</label>
        ...
        <div className="space-y-1">
          <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Centro de Custo</label>
          <div className="relative">
        ...
        <div className="space-y-1">
          <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Agrupamento</label>
          <div className="relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-emerald-50/50 border border-emerald-100 p-4 rounded-xl flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] text-emerald-700 font-bold uppercase tracking-wider">Entradas do Período</span>
            <span className="text-lg font-bold text-emerald-800 block">
              R$ {cashFlowTimeline.reduce((sum, item) => sum + item['Entradas Realizadas'] + item['Entradas Previstas'], 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </span>
            <span className="text-[9px] text-emerald-600 font-medium block">Total faturado + previsões</span>
          </div>
        ...
        <div className="bg-rose-50/50 border border-rose-100 p-4 rounded-xl flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] text-rose-700 font-bold uppercase tracking-wider">Saídas do Período</span>
            <span className="text-lg font-bold text-rose-800 block">
              R$ {cashFlowTimeline.reduce((sum, item) => sum + item['Saídas Realizadas'] + item['Saídas Previstas'], 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </span>
            <span className="text-[9px] text-rose-600 font-medium block">Total liquidado + agendamentos</span>
          </div>
        ...
        <div className="bg-zinc-50 border border-zinc-200 p-4 rounded-xl flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] text-zinc-600 font-bold uppercase tracking-wider">Saldo Final Previsto</span>
            <span className="text-lg font-bold text-zinc-900 block">
              R$ {(cashFlowTimeline[cashFlowTimeline.length - 1]?.['Saldo Acumulado'] || initialCash).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </span>
            <span className="text-[9px] text-zinc-500 font-medium block">Projeção estimada de fechamento</span>
          </div>
      ...
      {viewMode === 'chart' ? (
        <div className="bg-white p-5 rounded-xl border border-zinc-200 shadow-xs space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-zinc-900 uppercase tracking-wider">Simulação do Fluxo de Caixa no Período</h3>
            <span className="text-[10px] bg-zinc-100 text-zinc-500 font-medium px-2 py-0.5 rounded">Filtros Aplicados</span>
          </div>
          ...
          {cashFlowTimeline.length === 0 ? (
            ...
            <div className="h-96 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={cashFlowTimeline} margin={{ top: 10, right: 10, bottom: 0, left: 10 }}>
                  ...
                  <Bar dataKey="Entradas Realizadas" stackId="a" fill="#10b981" />
                  <Bar dataKey="Entradas Previstas" stackId="a" fill="#a7f3d0" />
                  <Bar dataKey="Saídas Realizadas" stackId="b" fill="#f43f5e" />
                  <Bar dataKey="Saídas Previstas" stackId="b" fill="#fecdd3" />
                  <Line type="monotone" dataKey="Saldo Acumulado" stroke="#09090b" strokeWidth={2} dot={false} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          )}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-zinc-50 border-b border-zinc-200">
                  <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">Período / Data</th>
                  <th className="p-4 text-xs font-bold text-emerald-700 uppercase tracking-wider text-right">Entradas Realizadas</th>
                  <th className="p-4 text-xs font-bold text-emerald-500 uppercase tracking-wider text-right">Entradas Previstas</th>
                  <th className="p-4 text-xs font-bold text-rose-700 uppercase tracking-wider text-right">Saídas Realizadas</th>
                  <th className="p-4 text-xs font-bold text-rose-400 uppercase tracking-wider text-right">Saídas Previstas</th>
                  <th className="p-4 text-xs font-bold text-zinc-800 uppercase tracking-wider text-right">Resultado Período</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-200 text-xs">
                {tableRows.map((row, idx) => {
                  ...
                  return (
                    <tr key={row.period || idx} className="hover:bg-zinc-50/80 transition-colors font-mono">
                      <td className="p-4 font-sans font-semibold text-zinc-900">
                        {periodType === 'monthly' ? (
                          new Date(row.period + '-02').toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })
                        ) : (
                          new Date(row.period).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' })
                        )}
                      </td>
                      <td className="p-4 text-emerald-700 text-right font-medium">
                        R$ {row.inRealized.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </td>
                      <td className="p-4 text-emerald-500 text-right">
                        R$ {row.inProjected.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </td>
                      <td className="p-4 text-rose-700 text-right font-medium">
                        R$ {row.outRealized.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </td>
                      <td className="p-4 text-rose-400 text-right">
                        R$ {row.outProjected.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </td>
                      <td className={`p-4 text-right font-bold ${diff >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
